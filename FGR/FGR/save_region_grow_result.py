...
def save_result(seq, output_dir, kitti_dataset_path, region_growth_config, args):
    # print("%s: begin" % seq)
    ...
    label_path = '../../GroundingDINO/lyft_2Dbox_txt/%s.txt'   % seq
    if not os.path.isfile(label_path):
        return

    img = cv2.imread(img_path)
    calib = kitti_utils_official.read_obj_calibration(calib_path)
    objects = kitti_utils_official.read_obj_data(label_path, calib, img.shape, args)
    ...
    for i in range(len(objects)):
        ...
        _, object_filter = kitti_utils_official.get_point_cloud_my_version(lidar_path, calib, img.shape, [objects[i].boxes_origin[0]], back_cut=False)
    ...
    for i in object_list:
        ...
        for j in range(thresh_seg_max):
            ...
            _, object_filter = kitti_utils_official.get_point_cloud_my_version(
                lidar_path, calib, img.shape, [objects[i].boxes_origin[0]], back_cut=False)
...
if __name__ == "__main__":
    ...
    for seq in seq_collection:
        # save_result(seq)
        pool.apply_async(save_result, (seq, args.output_dir, args.kitti_dataset_path, region_growth_config, args))
